...
public class MemberBrowser  {
	...
	public String memberMainView() {
		...
		while(true) {
			...
			case 100: remote = memberView(); // 회원 메인화면
				break;
			case 101: remote = personBrowser.info(); // 1>회원정보 화면
				break;
			...
			case 32:	remote = movieBrowser.opening_Movie();	// 3>무비차트 -> 2>개봉예정 화면
				break;
			case 41:	remote = ticketing.movieTimeChoice();	//  예매 1.날짜 화면
				break;
				}
			}
	}
	...
	public int memberView() {
		while (true) {
			...
			System.out.println("----------|1> 회원정보|2> 로그아웃|3>무비차트------------");
			System.out.println("-------------------|4> 나가기|----------------------");
			String menu = scanner.next();
			switch (menu) {
			case "1":  return Define.INFO;
